...
The server SHALL enforce API key authentication on all MCP endpoints. Every request SHALL include an `X-Api-Key` HTTP header whose value matches the configured server-side key. Requests without the header or with an incorrect value SHALL be rejected with HTTP 401 before reaching any MCP handler.

#### Scenario: Request with correct API key is allowed through
- **WHEN** a client sends a request to `/sse` with header `X-Api-Key: <correct-key>`
...
- **WHEN** a client sends a request to `/sse` without the `X-Api-Key` header
